...
async function getAllUsers(req, res) {
    try {
        const [rows] = await db.query('SELECT employee_id, full_name, username, role, created_at FROM employees ORDER BY employee_id DESC');
...
async function createUser(req, res) {
    try {
        ...
        if (exist.length > 0) {
            return res.status(400).json({ message: 'Tên đăng nhập hệ thống đã tồn tại' });
        }
        ...
        return res.json({ success: true, message: 'Tạo tài khoản người dùng thành công', id: r.insertId });
    } catch (err) {
...
async function updateUser(req, res) {
    try {
        ...
        return res.json({ success: true, message: 'Cập nhật tài khoản người dùng thành công' });
    } catch (err) {
...
async function deleteUser(req, res) {
    try {
        ...
        if (parseInt(userId) === req.user.id) {
            return res.status(400).json({ message: 'Không được phép tự xóa tài khoản chính mình đang đăng nhập' });
        }

        const [r] = await db.query('DELETE FROM employees WHERE employee_id = ?', [userId]);
        if (r.affectedRows === 0) {
            return res.status(404).json({ message: 'Không tìm thấy người dùng cần xóa' });
        }
        return res.json({ success: true, message: 'Xóa tài khoản người dùng thành công' });
    } catch (err) {
...
async function getUserById(req, res) {
    try {
        const [rows] = await db.query('SELECT employee_id, full_name, username, role, created_at FROM employees WHERE employee_id = ?', [req.params.id]);
        if (rows.length === 0) {
